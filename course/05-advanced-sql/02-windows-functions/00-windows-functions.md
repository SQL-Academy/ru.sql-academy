...
Однако, когда применяются оконные функции, для каждой строки определяется группа связанных с ней строк (или «окно»),
и по этой группе подсчитывается индивидуальное агрегатное значение.

Этот набор строк может быть:

- всей таблицей
- отдельными партициями таблицы, то есть группой строк на основе одного или нескольких полей
- или рамкой окна — конкретным диапазоном строк в пределах таблицы или партиции.
  Например, мы можем определить рамку окна, которая будет передаваться в оконную функцию,
  ...
  от строки к строке. Рамка будет как бы «скользить» по таблице.
...
![Изначальная таблица](https://sql-academy.org/static/guidePage/windows-functions/schema_table.png "Изначальная таблица")
...
- В качестве рамки окна можно указать и более специфический набор строк. Например, рамку можно определить как "предыдущая + текущая строка"
  таблицы. Тогда это будет выглядеть следующим образом:

    ![Схема формирования рамок окна](https://sql-academy.org/static/guidePage/windows-functions/4.png "Схема формирования рамок окна")

    Стоит отметить, что для первой строки рамка будет состоять только из 1-й записи, так как предыдущей строки нет.
...
- Далее следует `OVER`, который определяет окно (группу строк), которое будет передаваться в оконную функцию.
  Если конструкцию `OVER ()` оставить без параметров, то окном будут выступать все строки результата запроса.
...
- с помощью `ROWS|RANGE <определение_диапазона_строк>` формируются рамки окна. С помощью этого параметра можно указать, сколько строк брать до и после
  текущей строки.
...
ER-диаграмма базы данных Schedule: [открыть на SQL Academy](https://sql-academy.org/ru/guide/windows-functions).
...
можем увидеть на схеме ниже, оконные функции выполняются предпоследним шагом, уже после фильтрации и группировки, но
...
1. Оконные функции и агрегатные функции с группировкой выполняют одни и те же вычисления, но с использованием разного синтаксиса. — Оконные функции и агрегатные функции с группировкой имеют разную функциональность и не могут использоваться взаимозаменяемо.
