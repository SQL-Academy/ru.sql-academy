...
> Совместимые — значит, что СУБД может привести их к общему типу автоматически
> (например, `INT` и `BIGINT` будут приведены к `BIGINT`). Строгого совпадения типов не требуется.
