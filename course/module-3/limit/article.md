...
Если в запросе присутствует `ORDER BY`, то сначала выполняется сортировка результата, и только затем применяется ограничение по количеству строк.
Если `ORDER BY` отсутствует, порядок строк не гарантируется, и будут возвращены первые N строк в произвольном (фактически неопределённом) порядке.
