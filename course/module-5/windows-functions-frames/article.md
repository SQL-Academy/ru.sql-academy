...
1. ROWS и RANGE являются взаимозаменяемыми, поскольку оба определяют физические строки в окне — Это неверно, так как ROWS основан на физическом положении строк, в то время как RANGE ориентирован на значения столбцов.

2. **Правильный ответ:&#x20;**&#x52;OWS определяет границы окна на основе физического положения строк, а RANGE — на основе значений столбцов. — ROWS фокусируется на физическом положении строк, в то время как RANGE использует значения столбцов для определения границ окна.
...
4. И ROWS, и RANGE определяют границы окна исключительно на основе значений столбцов. — Это неверно, потому что только RANGE использует значения столбцов, в то время как ROWS ориентируется на физическое расположение строк.
