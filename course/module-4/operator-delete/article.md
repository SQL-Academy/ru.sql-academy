...
-   В MySQL сбрасывает счётчик идентификаторов до начального значения
-   Чтобы использовать, необходимы права на изменение таблицы

### PostgreSQL

-   В PostgreSQL счётчик идентификаторов сбрасывается только при использовании опции `RESTART IDENTITY`
-   По умолчанию используется `CONTINUE IDENTITY`, поэтому после `TRUNCATE` счётчик не сбрасывается
